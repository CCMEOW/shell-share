...
class ZYF_Main_MyTabBar: UIView {
    ...
    //闭包传值，创建一个闭包
    var clickBlock:((_ selcted:Int) ->())?
    override init(frame: CGRect) {
        super.init(frame: frame)
        self.frame = CGRect(x: 0, y: 0, width: width, height: 49)
    ...
    func creatTabBar(titNames:[String],imageNames:[String],selectedImageNames:[String],space:Int) {
        for i in 0...titNames.count - 1 {
            ...
            btn.frame = CGRect(x: 20 + CGFloat(i) * 83, y: 0, width: 49, height: 49)
            ...
            if i == 2{
                ...
                btn = ZYF_MyTabBarButton.init(frame: frame, image: "center")
                btn.frame = CGRect(x: 183, y: pointY, width: width / 5, height: pointX)
            } else {
                btn.setImage(selectedImage, for: .selected)
                btn.setImage(image, for: .normal)
            }
            btn.setTitle(titNames[i], for: .normal)
            self.addSubview(btn)
            btn.tag = 10 + i
